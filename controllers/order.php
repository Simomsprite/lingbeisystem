...
class Order extends IController implements adminAuthorization
{
    ...
    public function order_prescription_status()
    {
        //去掉左侧菜单和上部导航
        $this->layout='';
        $order_id = IFilter::act(IReq::get('id'),'int');
        ...
        $tb_order = new IModel('order');
        $updata= array();
        $updata['prescription_status'] = $type;
        $updata['completion_time'] =ITime::getDateTime();
        if($type == 2){
           $updata['status'] = 4;
        }
        $tb_order->setData($updata);
        $tb_order->update('id='.$order_id);
        ...
        {
            $action = '审核成功';
            $note   = '订单【'.$order_no.'】审核成功';

            $logObj = new log('db');
            $logObj->write('operation',array("管理员:".ISafe::get('admin_name'),"订单审核成功",'订单号：'.$order_no));
        }
        else
        {
            $action = '审核失败';
            $note   = '订单【'.$order_no.'】审核失败';
            $logObj = new log('db');
            $logObj->write('operation',array("管理员:".ISafe::get('admin_name'),"订单审核失败",'订单号：'.$order_no));
        }
	 ...
	 * @brief 退货操作
	 */
	public function order_return_save()
	{
		//获得post变量参数
		$order_id = IFilter::act(IReq::get('id'),'int');
		//发送的商品关联
		//$sendgoods = IFilter::act(IReq::get('sendgoods'));		
		$sendgoods=array();
		$result = Order_Class::return_order_operation($order_id,$sendgoods,$this->admin['admin_id']);
    ...
    public function order_update()
    {
    	...
    	foreach($goodsId as $key => $goods_id)
    	{
    		...
    		if($pid > 0)
    		{
                $cartValue['goods'][$key]['goods_id'] = $goods_id;
                $cartValue['goods'][$key]['type'] = "goods";
                $cartValue['goods'][$key]['is_cart'] = 1;
                $cartValue['goods'][$key]['car_id'] = 0;
                $cartValue['goods'][$key]['seller_id'] = 0;
                $cartValue['goods'][$key]['num'] = $num[$key];
    		}
    		else
    		{
                $cartValue['goods'][$key]['goods_id'] = $goods_id;
                $cartValue['goods'][$key]['product_id'] = $pid;
                $cartValue['goods'][$key]['type'] = "goods";
                $cartValue['goods'][$key]['is_cart'] = 1;
                $cartValue['goods'][$key]['car_id'] = 0;
                $cartValue['goods'][$key]['seller_id'] = 0;
		...
		$goodsResult  = $countSumObj->goodsCount($cartObj->cartFormat($cartValue));
